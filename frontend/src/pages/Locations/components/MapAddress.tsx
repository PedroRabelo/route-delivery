...
export function MapAddress({ locations, fetchLocations }: MapProps) {
  ...
  const [newBounds, setNewBounds] = useState<google.maps.LatLng>();
  ...
  function confirmChangeBounds(bounds: google.maps.LatLng) {
    setNewBounds(bounds);
    setOpenDeleteDialog(true);
  }

  async function handleChangeBounds() {
    if (locationSelected && newBounds !== undefined) {
      await api.patch(`pedidos-locais/${locationSelected.id}`, {
        latLongManual: true,
        latitude: newBounds.lat(),
        longitude: newBounds.lng()
      })
      fetchLocations()
      alert("Coordenadas alterada com sucesso")
      setNewBounds(undefined)
    }
  }
  ...
  return (
    <>
              ...
              {(clusterer): any =>
                locations?.map((point) => (
                  <Marker
                    ...
                    onDragEnd={(ev) => confirmChangeBounds(ev.latLng!)}
                    position={
                      {
      ...
      <DialogModal
        ...
        handleAction={() => handleChangeBounds()}
        title="Deseja alterar o endereço?"
      />
    </>
  );
}
